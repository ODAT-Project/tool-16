...
#residual block for the ResNet model
...
def cox_loss(log_risk, times, events):
    ...
    #calc log-sum-exp of risks for individuals at risk
    log_sum_exp_risk = torch.logcumsumexp(sorted_log_risk, dim=0)
    
    #select the terms for individuals who experienced an event
    observed_log_risk = sorted_log_risk[sorted_events]
    observed_log_sum_exp_risk = log_sum_exp_risk[sorted_events]
    
    #loss is the negative sum of log-partial-likelihoods
    loss = -torch.sum(observed_log_risk - observed_log_sum_exp_risk)
    
    return loss

class DynamicCVDApp:
    DL_RISK_SCORE_COL_NAME = 'ResNet_Risk_Score_Covariate'

    def __init__(self, root):
        self.root = root
        
        self.root.title("Advanced Dynamic CVD Risk Predictor -- ResNet-CPH Enhanced")
    ...
    def create_train_config_widgets(self, parent_frame):
        ...
        dl_fs_frame = ttk.LabelFrame(parent_frame, text="Features for ResNet Risk Score Model")
        
        dl_fs_frame.pack(fill=tk.BOTH, expand=True, pady=5)
        
        ttk.Label(dl_fs_frame, text="Select features for non-linear risk score (ResNet):").pack(anchor=tk.W, padx=5, pady=(5,0))
        ...
        dl_params_frame = ttk.LabelFrame(parent_frame, text="Risk Score Model (ResNet) Hyperparameters")
        ...
        self.train_button = ttk.Button(parent_frame, text="Train Hybrid ResNet-CPH Model", command=self.train_model_action)
    ...
    def _preprocess_features(self, df_subset, feature_names, stored_medians, scaler_to_use, scaled_cols_list, imputer_to_use, fit_transform=True):
        ...
        for col in processed_df.columns:
            ...
            if not processed_df[col].isnull().all():
                numeric_feature_names.append(col)
        
        
        processed_df = processed_df[numeric_feature_names]
    ...
    def train_model_action(self):
        #my original 2 stage hybrid model
        ...
        if not selected_dl_indices: messagebox.showerror("Error", "No features selected for ResNet Model."); return
        ...
        for col_list_name, col_list in [("ResNet", self.trained_dl_feature_names), ("CPH", self.trained_cph_linear_feature_names)]:
        ...
        try:
            ...
            if lr <= 0 or epochs <= 0 or cph_penalizer < 0 or not hidden_layers: raise ValueError("Hyperparameters out of valid range.")
        ...
        self.log_training_message("\n--- Stage 1: Training Risk Score Model (ResNet) ---")
        ...
        try:
            ...
            self.dl_model = ResNetModel(X_dl_processed.shape[1], hidden_layers)
            ...
            self.log_training_message(f"  Training ResNet for {epochs} epochs...")
            ...
            self.log_training_message("  ResNet model trained.")
            ...
            self.log_training_message(f"Error in ResNet training: {e}", is_error=True); self.log_training_message(traceback.format_exc(), is_error=True)
            messagebox.showerror("ResNet Training Error", f"Failed ResNet stage: {e}"); self.dl_model = None; return
    ...
    def generate_training_report(self, lr, epochs, hidden_layers, cph_pen, c_index):
        ...
        report_lines.append("--- Hybrid ResNet-CPH Model Training Report ---")
    ...
    def show_more_plots_window(self):
        ...
        try:
            y_train_sksurv = Surv.from_dataframe(event_col, time_col, train_df)
            y_test_sksurv = Surv.from_dataframe(event_col, time_col, test_df)
            ...
            valid_times = np.percentile(y_train_sksurv[y_train_sksurv[event_col]][time_col], [10, 80])
            ...
            self.log_training_message(f"Integrated AUC over time: {np.trapz(auc, times_to_eval) / (times_to_eval[-1] - times_to_eval[0]):.4f}")
    ...
    def show_about_dialog(self):
        messagebox.showinfo("About Advanced Dynamic CVD Predictor",
                            "Advanced Dynamic CVD Predictor -- ResNet-CPH Enhanced\n\n"
                            "A ResNet model generates a non-linear risk score. A CPH model incorporates this score along with other linear predictors for survival analysis.\n\n"
                            "Developed by ODAT project.")
